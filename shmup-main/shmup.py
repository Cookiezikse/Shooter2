...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.lives = 1 #Changer le nombres de vies
        self.hidden = False
...
def show_go_screen():
    ...
    draw_text(screen, "Piou Piou !", 64, WIDTH / 2, HEIGHT / 4)
    draw_text(screen, "Fleches directionnelles pour bouger, ESPACE pour tirer", 22, WIDTH / 2, HEIGHT / 2)
    draw_text(screen,"Appuye sur une touche pour commencer", 18,WIDTH/2, HEIGHT*3/4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
# load game graphics
background = pygame.image.load(os.path.join(img_folder, 'starfield2.png')).convert()
